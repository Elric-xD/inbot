...
async def init():
    if (
        ...
        LOGGER("AnonX").error(
            "Atleast add a pyrogram string, Existing..."
        )
        return
    if (
        ...
        LOGGER("AnonX").warning(
            "Fill SPOTIFY_CLIENT_ID & SPOTIFY_CLIENT_SECRET to play music from SPOTIFY"
        )
    try:
    ...
    try:
        ...
        LOGGER("AnonX").error(
            "[ERROR] - \n\nHey, Please Turn On Logger Group's Voice Call."
        )
        sys.exit()
    except:
        pass
