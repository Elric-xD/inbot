...
from config import (AUTO_DOWNLOADS_CLEAR, BANNED_USERS,
                    ...
                    MUSIC_BOT_NAME, confirmer, votemode, adminlist)
from AnonX import YouTube, app, userbot
...
from AnonX.utils.database.memorydatabase import get_upvote_count
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "UpVote":
        ...
        if get_upvotes >= upvote:
            ...
            try:
                ...
                return await CallbackQuery.edit_message_text(
                    f"ᴛʜɪs ᴠᴏᴛɪɴɢ ʜᴀs ᴇɴᴅᴇᴅ ʙᴇᴄᴀᴜsᴇ ᴛʜᴇ sᴛʀᴇᴀᴍ ᴇɴᴅᴇᴅᴏʀ sᴏᴍᴇᴏɴᴇ sᴛᴏᴘᴘᴇᴅ ɪᴛ ғᴏʀ ᴡʜɪᴄʜ ᴛʜɪs ᴠᴏᴛɪɴɢ ᴡᴀs sᴛᴀʀᴛᴇᴅ."
                )
            try:
                await CallbackQuery.edit_message_text(
                    f"» sᴜᴄᴄᴇssғᴜʟʟʏ ɢᴏᴛ **{upvote}** ᴠᴏᴛᴇs ғᴏʀ ᴘᴇʀғᴏʀᴍɪɴɢ ᴛʜᴀᴛ ᴀᴄᴛɪᴏɴ."
                )
            except:
                pass
            command = counter
    ...
    if command == "Pause":
        ...
        try:
            ...
            try:
                await CallbackQuery.edit_message_text(
                    f"» ꜱᴛʀᴇᴀᴍ ꜱᴋɪᴘᴘᴇᴅ ʙʏ {mention}", reply_markup=close_keyboard)
                await CallbackQuery.message.reply_text(
                    _["admin_10"].format(mention, CallbackQuery.message.chat.title), reply_markup=close_keyboard)
                return await Anon.stop_stream(chat_id)
            except:
                return
        await CallbackQuery.answer()
