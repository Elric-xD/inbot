...
{send}"""
    ...
    mystic = await message.reply_photo(
        photo=thumbnail, caption=cap, reply_markup=upl
    )
    if DUR != "Unknown":
        try:
...
async def queued_tracks(client, CallbackQuery: CallbackQuery, _):
    ...
    for x in got:
        j += 1
        if j == 1:
            msg += f'{config.MUSIC_BOT_NAME} ᴩʟᴀʏᴇʀ :\n\n📌 Title : {x["title"]}\nDuration : {x["dur"]}\nBy : {x["by"]}\n\n'
        elif j == 2:
            msg += f'Queued :\n\n✨ ᴛɪᴛʟᴇ : {x["title"]}\n🕒 ᴅᴜʀᴀᴛɪᴏɴ : {x["dur"]}\n🥀 ʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ : {x["by"]}\n\n'
        else:
            msg += f'✨ ᴛɪᴛʟᴇ : {x["title"]}\n🕒 ᴅᴜʀᴀᴛɪᴏɴ : {x["dur"]}\n🥀 ʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ : {x["by"]}\n\n'
...
{send}"""
    ...
    basic[videoid] = True
    thumbnail = await YouTube.thumbnail(videoid, True)
    med = InputMediaPhoto(media=thumbnail, caption=cap)
